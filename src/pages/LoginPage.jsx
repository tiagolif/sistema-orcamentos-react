  ...
  const handleSubmit = async (e) => {
    ...
    if (isRecovery) {
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: 'https://sistema-orcamentos-react.vercel.app/update-password',
      });

      if (error) {
        setError(error.message);
      } else {
        alert('Se o e-mail existir, você receberá um link para redefinir sua senha em instantes!');
        setIsRecovery(false); // Volta para a tela de login
      }
    } else {
      ...
      // O listener onAuthStateChange em App.jsx cuidará do redirecionamento em caso de sucesso
    }
    setLoading(false);
  };
    ...
    >
      <div className="w-full max-w-md p-8 space-y-6 bg-white bg-opacity-90 rounded-lg shadow-lg backdrop-blur-md">
        ...
        <p className="text-center text-gray-600">
          {isRecovery ? 'Digite seu e-mail para receber o link de recuperação.' : 'Faça login para continuar'}
        </p>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {loading ? 'Enviando...' : (isRecovery ? 'Enviar Link de Recuperação' : 'Entrar')}
            </button>
          </div>
        </form>
