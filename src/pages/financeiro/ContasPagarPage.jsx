...
import FormContasPagar from './FormContasPagar.jsx';
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th className="py-2 px-4 text-left w-[15%]">Fornecedor</th>
          ...
          <tbody>
            {lancamentos.map((lancamento) => (
              <tr key={lancamento.id}>
                ...
                <td className="py-2 px-4 text-sm">{lancamento.fornecedores ? (lancamento.fornecedores.razao_social || lancamento.fornecedores.nome_completo) : 'N/A'}</td>
                <td className="py-2 px-4 text-sm">{lancamento.obras ? lancamento.obras.nome_obra : 'N/A'}</td>
                <td className="py-2 px-4 text-sm text-center">
                  <div className="flex items-center justify-center space-x-1">
                    <Link to={`/financeiro/contas-a-pagar/visualizar/${lancamento.id}`} className="inline-block px-2 py-1 font-semibold rounded-md transition-all duration-200 ease-in-out text-secondary hover:bg-secondary/10 text-xs">Visualizar</Link>
