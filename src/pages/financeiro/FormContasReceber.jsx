...
const FormContasReceber = ({ isOpen, onClose, onSaveSuccess }) => {
  ...
  useEffect(() => {
    const fetchClientes = async () => {
      const { data } = await supabase.from('clientes').select('id, nome_completo, razao_social');
      setClientes(data);
    };
  ...
  const onSubmit = async (data) => {
    ...
    const submissionData = { ...data, tipo: 'RECEITA' };
    // Ensure optional IDs are null if empty string
    submissionData.cliente_id = submissionData.cliente_id || null;
    ...
    if (files.length > 0) {
      ...
      for (const file of files) {
        // 1. Pegue a extensão do arquivo (ex: 'png')
        const fileExt = file.name.split('.').pop();
        // 2. Crie um nome de arquivo novo e aleatório (ex: 'abc123xyz.png')
        const newFileName = `${Math.random().toString(36).substring(2)}.${fileExt}`;
        // 3. Crie o caminho de upload com o ID do lançamento e o NOVO nome
        const caminho = `${savedEntry.id}/${newFileName}`;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Novo Lançamento a Receber">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField label="Cliente" id="cliente_id">
        ...
        <FormField label="Obra" id="obra_id">
